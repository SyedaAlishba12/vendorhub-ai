...
async def list_vendors(
    ...
    return await vendor_controller.get_all_vendors(
        db, country, industry, certification, min_rating, search
    )
...
async def moderate_vendor(
    vendor_id: int, is_hidden: bool, db: AsyncSession = Depends(get_db)
):
...
async def feature_vendor(
    vendor_id: int, is_featured: bool, db: AsyncSession = Depends(get_db)
):

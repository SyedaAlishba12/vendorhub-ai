...
export default function DashboardPage() {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      const dashboardRes = await apiClient.get('/dashboard/buyer');
      setData(dashboardRes.data);

      // Fetch statistics (charts)
      try {
        const statsRes = await apiClient.get('/dashboard/statistics');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="bg-white p-6 rounded-2xl border border-rose-200 shadow-sm max-w-md w-full">
          <h2 className="text-sm font-bold text-rose-700">Error Loading Dashboard</h2>
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-indigo-100 mt-2 max-w-2xl">
              Your AI-powered procurement dashboard. Create RFQs, track quotations, and find verified suppliers instantly.
            </p>
            <QuickActions />
          </div>
        </div>
      ...
      <div className="bg-white p-2 rounded-2xl border border-slate-200/80 shadow-sm flex items-center gap-3">
        ...
        />
        <button type="button" className="p-2.5 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-100 transition-all">
          <SlidersHorizontal className="h-4 w-4" />
        </button>
        <button type="button" className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md shadow-indigo-600/20 transition-all shrink-0">
      ...
      {statistics && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <SpendingChart data={statistics.monthly_spending_trend} />
          <div className="grid grid-cols-2 gap-6">
            <RFQStatusChart data={statistics.rfq_status_distribution} />
            <OrderStatusChart data={statistics.order_status_distribution} />
          </div>
        </div>
      )}
      ...
      {data && (
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-slate-900">Revenue Insights</h2>
            <p className="text-[11px] text-slate-500 mt-0.5">Your total spend and savings opportunities</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-black text-slate-900">
              ${data.spending.total.toLocaleString()}
            </p>
            <p className="text-[10px] text-emerald-600 font-bold">+15% vs last year</p>
          </div>
        </div>
      )}
      ...
      {data && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <ActiveRFQsWidget rfqs={data.active_rfqs} />
            <PendingQuotationsWidget quotes={data.pending_quotations} />
            <RecentSearches searches={data.recent_searches} />
            <ActivityTimeline activities={data.activity} />
          </div>
          <div className="space-y-6">
            <OrdersSummary summary={data.orders_summary} />
            <SavedVendors vendors={data.saved_vendors} />
            <AIRecommendations recommendations={data.recommendations} />
            <SpendingSummary spending={data.spending} />
          </div>
        </div>
      )}

...
const [aiInsight, setAiInsight] = useState<{score:number; explanation:string}|null>(null);
const [aiLoading, setAiLoading] = useState(false);

const fetchAIInsight = async () => {
  setAiLoading(true);
  try {
    const res = await fetch(`http://localhost:8000/api/ai/vendors/${vendorId}/insight`);
    const data = await res.json();
    setAiInsight(data);
  } catch {
    setAiInsight({score:0, explanation:"AI unavailable"});
  } finally {
    setAiLoading(false);
  }
};
  ...
  <button
    ...
    disabled={aiLoading}
    className="px-3 py-1.5 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-200 text-xs font-bold hover:bg-indigo-100"
  >
    {aiLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin inline mr-1" /> : <Sparkles className="h-3.5 w-3.5 inline mr-1" />}
    Get AI Insight
  </button>
  {aiInsight && (
    <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-3 mt-2 text-xs text-slate-700">

...
export default function QuoteComparisonTable() {
  ...
  const rfqGroups = Array.from(new Set(quotes.map((q) => q.rfq_reference).filter(Boolean)));
  const currentQuotes = quotes.filter((q) => q.rfq_reference === selectedRfq);
  ...
  const bestDeliveryId = currentQuotes.length
    ? currentQuotes
        ...
        .reduce((a, b) => ((a.delivery_days ?? 999) < (b.delivery_days ?? 999) ? a : b), currentQuotes[0]).id
    : null;

  return (
    <AppShell activeHref="/quotes">
      <div>
        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Quote Comparison</h2>
                    ...
                    <tr key={q.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                      <td className="px-4 py-3 font-bold text-slate-800">
                        <div className="flex items-center gap-1.5">
                          ...
                          {q.id === bestDeliveryId && q.id !== bestPriceId && (
                      ...
                      <td className="px-4 py-3 text-slate-600 font-medium">{q.product_name || "-"}</td>
                      <td className="px-4 py-3 font-bold text-indigo-600">${q.price.toFixed(2)}</td>
                      ...
                      <td className="px-4 py-3 text-slate-600 font-medium">{q.payment_terms || "-"}</td>
                      <td className="px-4 py-3 text-slate-600 font-medium">
                        {q.warranty_months ? `${q.warranty_months} months` : "None"}
                      </td>
